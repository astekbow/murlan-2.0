...
const TYPE_LABEL: Record<MatchType, string> = { '1v1': '1 kundër 1', '1v1v1': '1v1v1', '2v2': '2 kundër 2' };
    ...
    <div className="fixed inset-x-0 bottom-4 z-[55] flex justify-center px-4 pointer-events-none">
      <div className="panel-solid pointer-events-auto flex items-center gap-3 px-4 py-3 max-w-md w-full animate-pop">
        ...
        <div className="flex-1 min-w-0 text-sm">
          <b className="text-gold-hi">{invite.fromUsername}</b> të ftoi në{' '}
          <b>{TYPE_LABEL[invite.type]}</b> · {dollars(invite.stakeCents)}
        </div>
        <button className="btn btn-ghost" onClick={dismiss}>Jo</button>
        <button className="btn btn-green" onClick={() => { sound.play('button'); void accept(); }}>Hyr</button>
      </div>
    </div>
  );
}
