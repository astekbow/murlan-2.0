...
const TYPE_LABEL: Record<string, string> = { '1v1': '1 kundër 1', '1v1v1': '1v1v1', '2v2': '2 kundër 2' };
...
export function SpectateView({ room }: { room: RoomStateDTO }) {
  ...
  return (
    <div className="relative z-10 mx-auto w-full max-w-[680px] px-3 pt-3 pb-6 space-y-4">
      <div className="flex items-center justify-between gap-3 animate-rise">
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-2xl">👁</span>
          <div className="min-w-0">
            <div className="font-display font-semibold text-gold-hi tracking-wide">PO SHIKON</div>
            <div className="text-xs text-muted truncate">{TYPE_LABEL[room.type] ?? room.type} · objektivi {room.target}</div>
          </div>
        </div>
        <button onClick={leave} className="btn btn-ghost shrink-0">Largohu</button>
      </div>
        ...
        {room.seats.map((s, i) => (
          <div key={i} className={`flex items-center justify-between gap-2 rounded-lg px-3 py-2 border transition-colors ${turn === i ? 'border-gold bg-gold/[.08]' : 'border-white/10'}`}>
            <div className="flex items-center gap-2 min-w-0">
              ...
              {room.type === '2v2' && s.team != null && <span className="text-[10px] text-muted">Ek. {s.team + 1}</span>}
              {turn === i && <span className="tag tag-live"><span className="pls" />Radha</span>}
              {passed.has(i) && <span className="text-[10px] text-muted">pas</span>}
              {finished.has(i) && <span className="text-[10px] text-emerald-300">mbaroi</span>}
            </div>
            <div className="text-sm text-muted shrink-0">{game ? `${game.handCounts[i] ?? 0} letra` : (s.connected ? 'gati' : 'jashtë')}</div>
          </div>
        ))}
      </section>
      ...
      {scoreboard && (
        <section className="panel p-4 animate-rise" style={{ animationDelay: '.15s' }}>
          <div className="font-serif text-[10px] tracking-[0.25em] text-muted uppercase mb-2">Rezultati · objektivi {scoreboard.target}</div>
          <div className="space-y-1">
            {scoreboard.type === '2v2' && scoreboard.teamTotals
              ? ([0, 1] as const).map((t) => (
                  <div key={t} className="flex justify-between text-sm"><span>Skuadra {t + 1}</span><b className="text-gold-hi">{scoreboard.teamTotals![t]}</b></div>
                ))
      ...
      {matchResult && (
        <div className="modal-backdrop !z-[60]" role="dialog" aria-modal="true" aria-label="Ndeshja mbaroi">
          <div className="panel-solid w-full max-w-sm p-7 text-center animate-pop">
            ...
            <h2 className="gold-text font-display font-bold tracking-wide text-2xl mb-1">Ndeshja mbaroi!</h2>
            <p className="text-sm text-muted mb-5">
              Fitoi {matchResult.winnerSeats.map((s) => nameOf(s)).join(' & ') || '—'}.
            </p>
            <button autoFocus onClick={leave} className="btn btn-gold btn-lg btn-block">Largohu</button>
          </div>
        </div>
      )}
    </div>
